...
public class ProductServiceImpl implements ProductService {
	...
	@Autowired
	private SubscribedProductRepository subscribeRepository;
	...
	@Override
	public String addProductSubscription(SubscribedProductDTO sProduct) {
		CustomPK custom = new CustomPK(sProduct.getBuyerId(),sProduct.getProdId());
		
		SubscribedProduct sp = new SubscribedProduct();
		
		sp.setCustomId(custom);
		sp.setQuantity(sProduct.getQuantity());
		
		int reward=100;
		if(reward>10000) {
			subscribeRepository.save(sp);
			return "Added Successfully";
		}
		else {
			return "Sorry ! you are not a privillaged user";
		}
	}
